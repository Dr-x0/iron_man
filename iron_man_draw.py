...
turtle.speed(2)
def draw_face(face_detilse_list,start_point) :
  turtle.penup()
  ...
  for i in range(len(face_detilse_list[0])) :
    x,y = face_detilse_list[0][i]
    turtle.goto(x,y)
  
  for i in range(len(face_detilse_list[1])) :
    x,y = face_detilse_list[1][i]
    turtle.goto(x,y)
  turtle.end_fill() 
